...
async def test_single_emission_decision_handler(systems, agents, announce):
    ...
    adapter = Adapter("auctioneer", systems, agents, 
                      emitter=mock_emitter, receiver=mock_receiver)
...
async def test_multiple_emissions_decision_handler(systems, agents, announce):
    ...
    async def multiple_emissions(forms):
        ...
        for announce_form in forms.messages(announce):
            ...
            messages.append(announce_form.bind(itemID="item1", description="A valuable item"))
            messages.append(announce_form.bind(itemID="item2", description="Another item"))
            messages.append(announce_form.bind(itemID="item3", description="Final item"))
            return messages
