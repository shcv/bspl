#!/usr/bin/env python3
"""
Python implementation of the test script for the 3-enablement exercise.
...
class EnablementTester:
    def __init__(self):
        ...
        if self.is_solution_mode:
            ...
            self.buyer_script = self.script_dir / "buyer_enabled.py"
            self.seller_script = self.script_dir / "seller_enabled.py"
        else:
        ...
        self.buyer_log = self.logs_dir / "buyer_enabled.log"
        self.seller_log = self.logs_dir / "seller_enabled.log"
    ...
    def parse_arguments(self):
        ...
        parser = argparse.ArgumentParser(description="Run Forms-Based Purchase Protocol tests")
    ...
    def start_agents(self):
        ...
        # Start Seller
        print(f"Starting Seller (forms-based) from: {self.seller_script}")
        seller_process = subprocess.Popen(
            ["python", str(self.seller_script)],
        ...
        # Start Buyer
        print(f"Starting Buyer (forms-based) from: {self.buyer_script}")
        buyer_process = subprocess.Popen(
            ["python", str(self.buyer_script)],
        ...
        print("Running forms-based purchase protocol for 3 seconds...")
        time.sleep(3)
    ...
    def run_tests(self):
        ...
        self.check_pattern("Starting Buyer agent.*forms-based", "Buyer agent (forms-based) started")
        self.check_pattern("Starting Seller agent.*forms-based", "Seller agent (forms-based) started")
        
        # Check for message transmissions in the debug logs
        self.check_pattern("Received message: .*RFQ", "Seller received RFQ messages")
        self.check_pattern("Sending .*Quote", "Seller sent Quote messages")
        self.check_pattern("Received message: .*Quote", "Buyer received Quote messages")
