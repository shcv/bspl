...
BUYER_SCRIPT="buyer.py"  # Default: solution buyer script
SELLER_SCRIPT="seller.py"  # Default: solution seller script

# Setup PYTHONPATH to ensure modules can be found (safely handle if unset)
...
while [ $# -gt 0 ]; do
  ...
  # Use case statement for better compatibility
  case "$filename" in
    *buyer*)
      BUYER_SCRIPT="$arg"
...
# Log files
BUYER_LOG="$SCRIPT_DIR/logs/buyer.log"
SELLER_LOG="$SCRIPT_DIR/logs/seller.log"
...
# Use case statement for better compatibility
case "$SELLER_SCRIPT" in
  /*)
    # Absolute path
    echo "Starting Seller from: $SELLER_SCRIPT"
  ...
  ./*)
    ...
    echo "Starting Seller from: $SELLER_SCRIPT (relative to $SCRIPT_DIR)"
    cd "$SCRIPT_DIR"
    export PYTHONPATH="$SCRIPT_DIR:${PYTHONPATH:-}"
    BSPL_ADAPTER_DEBUG=true python "$SELLER_SCRIPT" > $SELLER_LOG 2>&1 &
    cd "$START_DIR"
    ;;
  ../*)
    # Relative path up from solution directory
    echo "Starting Seller from: $SELLER_SCRIPT (relative to $SCRIPT_DIR)"
    cd "$SCRIPT_DIR"
    export PYTHONPATH="$SCRIPT_DIR:${PYTHONPATH:-}"
    BSPL_ADAPTER_DEBUG=true python "$SELLER_SCRIPT" > $SELLER_LOG 2>&1 &
    ...
    ;;
  *)
    # Default case - script name only
    echo "Starting Seller from solution directory"
    cd "$SCRIPT_DIR"
    export PYTHONPATH="$SCRIPT_DIR:${PYTHONPATH:-}"
    BSPL_ADAPTER_DEBUG=true python "$SELLER_SCRIPT" > $SELLER_LOG 2>&1 &
    cd "$START_DIR"
...
# Use case statement for better compatibility
case "$BUYER_SCRIPT" in
  /*)
    # Absolute path
    echo "Starting Buyer from: $BUYER_SCRIPT"
  ...
  ./*)
    # Relative path to current directory
    echo "Starting Buyer from: $BUYER_SCRIPT (relative to $SCRIPT_DIR)"
    cd "$SCRIPT_DIR"
    export PYTHONPATH="$SCRIPT_DIR:${PYTHONPATH:-}"
    BSPL_ADAPTER_DEBUG=true python "$BUYER_SCRIPT" > $BUYER_LOG 2>&1 &
    cd "$START_DIR"
  ...
  *)
    # Default case - script name only
    echo "Starting Buyer from solution directory"
    cd "$SCRIPT_DIR"
    export PYTHONPATH="$SCRIPT_DIR:${PYTHONPATH:-}"
    BSPL_ADAPTER_DEBUG=true python "$BUYER_SCRIPT" > $BUYER_LOG 2>&1 &
    cd "$START_DIR"
...
echo "Running purchase protocol for 3 seconds..."
sleep 3
...
check_pattern "Starting Buyer agent" "Buyer agent started" || ((FAILURES++))
check_pattern "Starting Seller agent" "Seller agent started" || ((FAILURES++))
...
check_pattern "Sending .*Quote" "Seller sent quotes" || ((FAILURES++))
check_pattern "Received message: .*Quote" "Buyer received quotes" || ((FAILURES++))
