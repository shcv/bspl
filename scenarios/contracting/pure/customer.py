...
# track how many invitations we've made
bidders = 0
...
async def init(enabled):
    ...
    for m in enabled.messages:
        if m.schema == Invite and bidders < 9:
            m.bind(contractID=1, bidID=bidders, spec="mobile app")
            bidders += 1
            logging.info(f"inviting bidder: {m}")
            return [m]
